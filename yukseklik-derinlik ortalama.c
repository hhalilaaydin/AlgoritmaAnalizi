...
if(agac==NULL){
printf(Heapte gerekli yer ayrilamadi... exit ...n);
exit(1);
}
(agac)-kok=NULL;
}
...
printf(random sayisi=%dn,randomSayi);
for(n=0;nrandomSayi;n++){
...
printf(Ic dugum sayisi %4dn,foo(agac-kok));
printf(derinlik sayisi %4dn,derinlik(agac-kok));
int a;
int i;
float toplam=0;
for(i=0;itoplamEleman;i++)
{
toplam+=dizi[i];
}
printf(Derinlik Ortalamasi%fn,toplamtoplamEleman);
scanf(%d, &a);
return (EXIT_SUCCESS);
}
